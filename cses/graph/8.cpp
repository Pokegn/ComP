...
#include <bits/stdc++.h> //dijkstra para llegar de un vertice a todos con distancias positivas, en O(mlog(m)) con minheap (n=1e5, m=2e5)
using namespace std;
...
void solve(){
    ...
    minheap<pair<lli, lli>> minh; minh.push({0,0}); 
    vlli ans(n, -1);
    ...
    while(!minh.empty()){
        ...
        ans[v.second] = v.first; //v.first tiene la menor distancia 

        for(auto &u: grafo[v.second]){ // a lo mas m veces agregamos cosas
            minh.push({ans[v.second] + u.first, u.second}); //log m
        } 
    }

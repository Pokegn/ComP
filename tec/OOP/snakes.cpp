...
public:
  InvalidConfigurationException(const string& param_name, int param_value) {
    message = "Invalid " + param_name + " value " + to_string(param_value);
  }
...
public:
  InvalidOptionException() {
    message = "Invalid option, please press C to continue next turn or E to end the game";
  }
...
public:
  ...
  bool playTurn() override {
    ...
    int attempts = 0;
    while (attempts < 5) {
      cout << "Press C to roll dice, or E to end: ";
      char choice;
      cin >> choice;
      if (toupper(choice) == 'E') {
        ...
        try {
          ...
          cout << e.what() << endl;
          if (attempts >= 5) {
            cout << "-- GAME OVER -- \n Invalid menu choice exceeded" << endl;
            return true;
          }
        }
      }

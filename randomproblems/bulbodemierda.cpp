...
void solve(){
    ...
    rep(i, 1, n+1){
        ...
        ll bestidx = 0; //mejor idx de donde estaba antes
        for(int j = 1; j<2*n+1; j++){ //costidx es el costo de moverlo de idx a j, costcurr es el costo de moverlo de j a j
            ll costidx = dp[i-1][bestidx] + abs(mr[bestidx] - mr[j]);
            bestder[j] = costidx;
            //ll costcurr = dp[i-1][j] + cost(mr[j], a[i].fi, a[i].se);
            if(dp[i][j] < costidx) bestidx = j;
        }

        //o me muevo a la izquierda
        bestidx = 2*n; //mejor idx de donde estaba antes
